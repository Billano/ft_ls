...
/*   Updated: 2017/06/07 22:40:16 by eurodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char						ft_flags_parser(int ac, char **av, int *i,\
		t_ls_flags *flags);
...
void						ft_store_other_files(char **av, int i, t_ls_flags\
		flags, t_str_tree **other_files);
void						ft_store_dir_files(char **av, int i, t_ls_flags\
		flags, t_str_tree **dir_files);
void						ft_print_other_files(t_str_tree *root,\
		t_ls_flags flags, t_ls_permisions *ls_p);
void						ft_print_dir_files(t_str_tree *dir_files,\
		t_str_tree *e_files, t_str_tree *other_files, t_ls_flags flags);
...
int							ft_ls_time_compare(t_avl_tree_ls *root,\
		t_ls_data *ls_data);
void						ft_print_open_dir_b(t_str_tree *root,\
		t_ls_flags flags);
...
void						ft_print_other_files_r(t_str_tree *root,\
		t_ls_flags flags, t_ls_permisions *ls_p);
char						ft_is_symlink_dir(char *path);
void						ft_ls_print_long(t_avl_tree_ls *root,\
		t_ls_permisions *ls_p, t_ls_flags flags);
void						ft_avl_tree_ls_inorder(t_avl_tree_ls *root,\
		char *path, t_ls_flags flags, t_ls_permisions *ls_p);
void						ft_avl_tree_ls_backorder(t_avl_tree_ls *root,\
		char *path, t_ls_flags flags, t_ls_permisions *ls_p);
void						ft_avl_tree_ls_r_inorder(t_avl_tree_ls *root,\
		char *path, t_ls_flags flags);
void						ft_avl_tree_ls_r_backorder(t_avl_tree_ls *root,\
		char *path, t_ls_flags flags);
...
t_ls_data					*ft_ls_data_init(char *d_name, struct stat f_stat,\
		char *path);
t_str_tree					*ft_str_tree_create(char *data);
t_str_tree					*ft_str_tree_insert(t_str_tree *root, char *data);
t_str_tree					*ft_str_tree_time_insert(t_str_tree *root,\
		char *path);
void						ft_print_open_dir(t_str_tree *root, t_ls_flags\
		flags, char *str);
void						ft_print_reglk_files(t_str_tree *root,\
		t_ls_flags flag, t_ls_permisions *ls_p);
...
t_avl_tree_ls				*ft_store_dir(DIR *fd, t_ls_flags flags,\
		char *path, t_ls_permisions *ls_p);
void						ft_avl_tree_ls_r_inorder(t_avl_tree_ls *root,\
		char *path, t_ls_flags flags);
void						ft_avl_tree_ls_r_backorder(t_avl_tree_ls *root,\
		char *path, t_ls_flags flags);
int							ft_is_node_a_dir(t_avl_tree_ls *root);
void						ft_permissions_update(t_avl_tree_ls *root,\
		t_ls_permisions *ls_p);
char						*ft_file_permissions(t_ls_data *ls_data,\
		char *path);
