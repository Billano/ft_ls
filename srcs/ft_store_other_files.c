...
/*   Updated: 2017/06/06 02:54:49 by eurodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_str_tree **other_files)
{
	while (av[i])
	{
		...
		(ft_is_link(av[i]) && !ft_is_symlink_dir(av[i])) || ft_is_socket(av[i]) || ft_is_fifo(av[i])\
		|| ft_is_reg_file(av[i]))
			*other_files = flags.t_flag ? ft_str_tree_insert(*other_files,\
